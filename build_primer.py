        ...
        if len(record) >= 2:
            ...
            if do_aln:
                ...
                try:
                    ...
                    subprocess.run([muscle_path, "-align", seq_file, "-output", aln_file], shell=True, check=True)
            ...
            if sys.platform.startswith('win'):
                primer3_path = os.path.join(script_dir, "primer3_core.exe")
            elif sys.platform == 'darwin':
                raise RuntimeError("TODO") # TODO
            else:
                primer3_path = os.path.join(script_dir, 'primer3_core')

            primer_command = [primer3_path, "--p3_settings_file", setting_file ,"--output", primer_file , input_file]
            try:
...
if __name__ == '__main__':
    ...
    pars = argparse.ArgumentParser(
        formatter_class=argparse.RawDescriptionHelpFormatter, description='''Build primers for multiple sequences.''')
    ...
    pars.add_argument('-soft_boundary', metavar='<int>', type=int,
                      help='''soft boundary length''', required=False, default=200)
    ...
    pars.add_argument('-max_seq_length', metavar='<int>', type=int,
                      help='''max gene length''', required=False, default=5000)
    pars.add_argument('-min_seq_length', metavar='<int>', type=int,
                      help='''min gene length''', required=False, default=200)
    pars.add_argument('-gap_length', metavar='<int>', type=int,
                      help='''min gap length''', required=False, default=100)
    pars.add_argument('-t', metavar='<int>', type=int,
                      help='''thread count''', required=False, default=16)
    pars.add_argument('-usevar', type=str2bool, nargs='?', const=True, help='''whether to design primers using only consensus sites''', default = True)
    pars.add_argument('-split_only', type=str2bool, nargs='?', const=True, help='''split sequences instead of actually designing primers''', default = False)
    pars.add_argument('-out_dir', metavar='<str>', type=str,
                      help='''output directory''', required=True)
    pars.add_argument('-max_primer', metavar='<int>', type=int,
                      help='''max number of primers''', required=False, default=64)
