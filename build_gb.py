...
if __name__ == "__main__":
    pars = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description='''Convert GenBank database files to individual GenBank sequences and a CSV table.''')
    pars.add_argument('-input', metavar='<str>', type=str, help='''input GenBank database''', required=True)
    pars.add_argument('-outdir', metavar='<str>', type=str, help='''output directory for individual GenBank files''', required=True)
    pars.add_argument('-clean', type=str2bool, nargs='?', const=True, help='''whether to clear existing data (dangerous!) from the output directory''', default=False)
    pars.add_argument('-outcsv', metavar='<str>', type=str, help='''output csv file''', required=True)
